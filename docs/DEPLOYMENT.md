...
> **重要**: インストールスクリプトでは `libcamerify` を含む必要な依存パッケージを自動的にインストール／確認します（Step 1 の初期セットアップを完了していれば、追加の手動インストールは通常不要です）。もしスクリプト実行時に依存パッケージ関連のエラーが出る場合は、`libcamerify` が正しくインストールされているかを確認してください。

> [!WARNING]
> **所要時間に関する注意**: Raspberry Pi Zero の性能上、`uv sync` による Python 依存関係（特に `numpy`）のインストールには **非常に長い時間（2〜4時間以上）** がかかります。これは ARMv6l 向けのバイナリホイールが存在せず、ソースからコンパイルされるためです。画面が長時間更新されなくても、ハングアップしているわけではありませんので、電源を切らずにお待ちください。
