...
```bash
#!/bin/bash
# /home/pi/chigemotsu/production/scripts/chigemotsu_detect.sh

# 引数として画像パスを受け取る
IMAGE_PATH="$1"

# ログファイル
LOG_FILE="/home/pi/chigemotsu/production/logs/motion_integration.log"

# 現在時刻をログに記録
echo "$(date): Motion detected, processing image: $IMAGE_PATH" >> "$LOG_FILE"

# 仮想環境をアクティベート（必要に応じて）
cd /home/pi/chigemotsu/production
...
```bash
# スクリプトを作成
sudo nano /home/pi/chigemotsu/production/scripts/chigemotsu_detect.sh

# 実行権限を付与
chmod +x /home/pi/chigemotsu/production/scripts/chigemotsu_detect.sh

# ログディレクトリを作成
mkdir -p /home/pi/chigemotsu/production/logs
```
...
# chigemotsu統合ログを確認
tail -f /home/pi/chigemotsu/production/logs/motion_integration.log
...
3. **ファイルパスの問題**
   ...
   # 設定ファイルの確認
   cat /home/pi/chigemotsu/production/config/config.json
   ```

### ログ確認

- **Motion統合ログ**: `/home/pi/chigemotsu/production/logs/motion_integration.log`
- **chigemotsu検出ログ**: `/home/pi/chigemotsu/production/logs/chigemotsu_detection.log`
- **LINE通知ログ**: `/home/pi/chigemotsu/production/logs/line_image_notifier.log`
- **R2アップロードログ**: `/home/pi/chigemotsu/production/logs/r2_uploader.log`
...
Type=simple
User=pi
ExecStart=/bin/bash -c "tail -f /home/pi/motion_images/*.jpg | while read file; do /home/pi/chigemotsu/production/scripts/chigemotsu_detect.sh \"$file\"; done"
Restart=always
